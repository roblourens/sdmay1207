...
public class H263Packetizer extends Thread implements Runnable {
	...
	// sets up RTP socket
	public H263Packetizer(InputStream in, InetAddress dest, int port) throws SocketException {
		...
		endOfFrame = true;
		cycle = 0;
		Log.d(PhotosActivity.LOG_TAG,"Sending to "+dest.toString()+":"+port);
       ...
       try {
    	   ...
    	   while(running){  		  
    		   ...
    		   if(length > 0) {
    			   // Store length read
    			   totalBytes += length;
    			   System.out.println("Bytes read: "+length+"\tTotal bytes: "+totalBytes);
    			   length += rtphl+2;
	...
	private int readNextFrame(){
		...
		try{
			...
			len = fis.read(streamBuffer,leftoverAmount,streamBuffer.length-leftoverAmount-1);
			
			if(len>0){
				// Get index of next end of frame
				index = nextEOF();
				
				// If no end of frame found, then copy all of data read into buffer
				if(index >= leftoverAmount + len-2){
					System.arraycopy(streamBuffer, 0, buffer, rtphl+2, leftoverAmount+len-2);
					length = len;
					leftoverAmount = 2;
	
				} else {
					// Copy frame into buffer and mark next packet as end of frame
					System.arraycopy(streamBuffer, 0, buffer, rtphl+2, index);
					length = index;
					rsock.markNextPacket();
					
					// Mark leftover and move leftover to beginning of stream buffer
					leftoverAmount = leftoverAmount + len-(index+2);
					System.arraycopy(streamBuffer, index+2, streamBuffer, 0, leftoverAmount);
					endOfFrame = true;
				} 
			}
		} catch (IOException e){
	...
	private int nextEOF(){
		...
		while(index < streamBuffer.length - 2){
			// Check if two zeros in a row, then check if the third is 0x80/0x81/0x82/0x83
			if(streamBuffer[index] == eofBytes[0] && streamBuffer[index+1] == eofBytes[1]){
